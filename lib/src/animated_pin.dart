...
class _AnimatedLocationPinState extends State<AnimatedLocationPin> with TickerProviderStateMixin {
  ...
  bool get _currentElevated => _isAdvancedMode ? widget.state == PinState.elevated : widget.isElevated;
  ...
  void initState() {
    ...
    _bounceController = AnimationController(
      duration: _isAdvancedMode ? const Duration(milliseconds: 450) : const Duration(milliseconds: 400),
      vsync: this,
    );
  ...
  void _setupAnimations() {
    if (_isAdvancedMode) {
      ...
      _squashAnimation = TweenSequence<double>([
        ...
        TweenSequenceItem(
          tween: Tween<double>(begin: 1.35, end: 0.92).chain(CurveTween(curve: Curves.bounceOut)),
          weight: 25,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 0.92, end: 1.08).chain(CurveTween(curve: Curves.easeInOut)),
          weight: 30,
        ),
        TweenSequenceItem(
      ...
      _stretchAnimation = TweenSequence<double>([
        TweenSequenceItem(
          tween: Tween<double>(begin: 1.0, end: 0.65).chain(CurveTween(curve: Curves.easeOut)),
          weight: 15,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 0.65, end: 1.15).chain(CurveTween(curve: Curves.bounceOut)),
          weight: 25,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 1.15, end: 0.96).chain(CurveTween(curve: Curves.easeInOut)),
          weight: 30,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 0.96, end: 1.0).chain(CurveTween(curve: Curves.easeInOut)),
          weight: 30,
        ),
      ]).animate(_bounceController);
      ...
      _shadowDistance = Tween<double>(
        begin: 0.0,
        end: widget.shadowDistance,
      ).animate(CurvedAnimation(parent: _liftController, curve: Curves.easeOut));
    } else {
      ...
      _squashAnimation = TweenSequence<double>([
        ...
        TweenSequenceItem(
          tween: Tween<double>(begin: 1.3, end: 0.95).chain(CurveTween(curve: Curves.easeIn)),
          weight: 20,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 0.95, end: 1.05).chain(CurveTween(curve: Curves.easeOut)),
          weight: 30,
        ),
        TweenSequenceItem(
      ...
      _stretchAnimation = TweenSequence<double>([
        TweenSequenceItem(
          tween: Tween<double>(begin: 1.0, end: 0.7).chain(CurveTween(curve: Curves.easeOut)),
          weight: 20,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 0.7, end: 1.1).chain(CurveTween(curve: Curves.easeIn)),
          weight: 20,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 1.1, end: 0.98).chain(CurveTween(curve: Curves.easeOut)),
          weight: 30,
        ),
        TweenSequenceItem(
          tween: Tween<double>(begin: 0.98, end: 1.0).chain(CurveTween(curve: Curves.easeInOut)),
          weight: 30,
        ),
      ]).animate(_bounceController);
    }
  ...
  Widget build(BuildContext context) {
    final stickWidth = widget.stickWidth ?? widget.size * 0.15;
    ...
    if (_isAdvancedMode) {
      ...
      return AnimatedBuilder(
        ...
        builder: (context, child) {
          ...
          final shadowWidth = widget.state == PinState.elevated ? stickWidth * 1.2 : stickWidth * 0.8;
          final shadowHeight = widget.state == PinState.elevated ? stickWidth * 0.85 : stickWidth * 0.25;
          final shadowOpacity = widget.state == PinState.elevated ? _shadowOpacity.value : _shadowOpacity.value * 0.5;

          return Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              // The pin with bounce effect
              Transform.scale(scaleX: _squashAnimation.value, scaleY: _stretchAnimation.value, child: _buildPin()),

              // Shadow with smooth distance animation
              SizedBox(height: _shadowDistance.value),
              Container(
                width: shadowWidth * 0.8,
                height: shadowHeight * 0.8,
                decoration: BoxDecoration(
                  color: widget.shadowColor.withValues(alpha: shadowOpacity),
                  borderRadius: BorderRadius.circular(999),
                  boxShadow: [
                    BoxShadow(
                      color: widget.shadowColor.withValues(alpha: shadowOpacity * 0.4),
                      blurRadius: 8,
                      spreadRadius: 8,
                    ),
                  ],
      ...
      return Column(
        ...
        children: [
          ...
          AnimatedBuilder(
            ...
            builder: (context, child) {
              return Transform.scale(scaleX: _squashAnimation.value, scaleY: _stretchAnimation.value, child: child);
            },
            child: _buildPin(),
          ),
          ...
          AnimatedContainer(
            ...
            width: widget.isElevated ? stickWidth * 0.2 : stickWidth * 0.8,
            height: widget.isElevated ? stickWidth * 2 : stickWidth * 0.25,
            decoration: BoxDecoration(
  ...
  Widget _buildPin() {
    final stickWidth = widget.stickWidth ?? widget.size * 0.1;
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          Positioned(
            bottom: 0,
            left: (widget.size - stickWidth) / 2,
            child: Container(
              width: stickWidth,
              height: widget.stickHeight * 1.2,
              decoration: BoxDecoration(
                color: widget.stickColor ?? widget.color,
                borderRadius: BorderRadius.only(
                  bottomLeft: Radius.circular(widget.stickBorderRadius),
                  bottomRight: Radius.circular(widget.stickBorderRadius),
                ),
              ),
            ),
          ),
            ...
            child: Container(
              ...
              child: Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(color: widget.innerColor, shape: BoxShape.circle),
                ),
              ),
            ),
          ),
